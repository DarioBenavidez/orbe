...
// ── Importar server ────────────────────────────────────────
const { processAction, interpretMessage, defaultData, fmt, today } = require('./server.js');
...
await test('Transacciones', 'agregar gasto', async () => {
  const r = await processAction({ type:'agregar_transaccion', txType:'gasto', description:'Almuerzo', amount:3000, category:'Comida', date:today() }, mockData(), UID, NAME, [], PHONE);
  assertStr(r); assertIncludes(r, '3.000');
});

await test('Transacciones', 'agregar ingreso', async () => {
  const r = await processAction({ type:'agregar_transaccion', txType:'ingreso', description:'Freelance', amount:50000, category:'Ingreso', date:today() }, mockData(), UID, NAME, [], PHONE);
  assertStr(r); assertIncludes(r, '50.000');
});

await test('Transacciones', 'agregar sueldo no crashea', async () => {
  const r = await processAction({ type:'agregar_transaccion', txType:'sueldo', description:'Sueldo', amount:1500000, category:'Ingreso', date:today() }, mockData(), UID, NAME, [], PHONE);
  assertStr(r);
});

await test('Transacciones', 'alerta presupuesto al 80%', async () => {
  const r = await processAction({ type:'agregar_transaccion', txType:'gasto', description:'Super', amount:85000, category:'Comida', date:today() }, mockData({ budgets:[{ cat:'Comida', limit:100000 }] }), UID, NAME, [], PHONE);
  assertIncludes(r, '%');
});

await test('Transacciones', 'alerta presupuesto al 100%', async () => {
  const r = await processAction({ type:'agregar_transaccion', txType:'gasto', description:'Super', amount:110000, category:'Comida', date:today() }, mockData({ budgets:[{ cat:'Comida', limit:100000 }] }), UID, NAME, [], PHONE);
  assertIncludes(r, 'Pasaste');
});

await test('Transacciones', 'últimas transacciones con datos', async () => {
  const r = await processAction({ type:'ultimas_transacciones' }, mockData(), UID, NAME);
  assertStr(r); assertIncludes(r, 'Últimas');
});

await test('Transacciones', 'últimas transacciones vacío', async () => {
  const r = await processAction({ type:'ultimas_transacciones' }, mockData({ transactions:[] }), UID, NAME);
  assertIncludes(r, 'No hay');
});

// ── Balance y Resumen ──────────────────────────────────────
await test('Balance', 'consultar_balance muestra ingresos y gastos', async () => {
  const r = await processAction({ type:'consultar_balance' }, mockData(), UID, NAME);
...
await test('Balance', 'resumen_general completo', async () => {
  const r = await processAction({ type:'resumen_general' }, mockData(), UID, NAME);
...
await test('Presupuestos', 'consultar_presupuesto con datos', async () => {
  const r = await processAction({ type:'consultar_presupuesto' }, mockData(), UID, NAME);
  assertStr(r);
});

await test('Presupuestos', 'consultar_presupuesto vacío', async () => {
  const r = await processAction({ type:'consultar_presupuesto' }, mockData({ budgets:[] }), UID, NAME);
  assertIncludes(r, 'No');
});

await test('Presupuestos', 'consultar_presupuesto_categoria existente', async () => {
  const r = await processAction({ type:'consultar_presupuesto_categoria', category:'Comida' }, mockData(), UID, NAME);
...
await test('Presupuestos', 'consultar_presupuesto_categoria inexistente', async () => {
  const r = await processAction({ type:'consultar_presupuesto_categoria', category:'Viajes' }, mockData(), UID, NAME);
  assertIncludes(r, 'presupuesto');
});

await test('Presupuestos', 'actualizar_presupuesto nuevo', async () => {
  const r = await processAction({ type:'actualizar_presupuesto', category:'Ropa', limit:30000 }, mockData(), UID, NAME);
  assertIncludes(r, 'Ropa'); assertIncludes(r, '30.000');
});

// ── Eventos ───────────────────────────────────────────────
await test('Eventos', 'agregar_evento', async () => {
  const r = await processAction({ type:'agregar_evento', title:'Pago Gas', day:15, eventType:'vencimiento', notify:true }, mockData(), UID, NAME);
  assertIncludes(r, 'Pago Gas'); assertIncludes(r, '15');
});

await test('Eventos', 'eliminar_evento existente', async () => {
  const r = await processAction({ type:'eliminar_evento', keyword:'Visa' }, mockData(), UID, NAME);
  assertIncludes(r, 'elimin');
});

await test('Eventos', 'eliminar_evento inexistente', async () => {
  const r = await processAction({ type:'eliminar_evento', keyword:'XYZ' }, mockData(), UID, NAME);
  assertIncludes(r, 'encontr');
});

await test('Eventos', 'consultar_vencimientos', async () => {
  const r = await processAction({ type:'consultar_vencimientos' }, mockData(), UID, NAME);
  assertStr(r);
});

// ── Préstamos (te deben) ──────────────────────────────────
await test('Préstamos', 'agregar_prestamo', async () => {
  const r = await processAction({ type:'agregar_prestamo', name:'Pedro', amount:5000, reason:'cena' }, mockData(), UID, NAME);
  assertIncludes(r, 'Pedro'); assertIncludes(r, '5.000');
});

await test('Préstamos', 'registrar_pago parcial', async () => {
  const r = await processAction({ type:'registrar_pago_prestamo', name:'Claudio', amount:3000 }, mockData(), UID, NAME);
  assertIncludes(r, 'Claudio'); assertIncludes(r, '3.000');
});

await test('Préstamos', 'registrar_pago que salda', async () => {
  const r = await processAction({ type:'registrar_pago_prestamo', name:'Claudio', amount:8000 }, mockData(), UID, NAME);
  assertIncludes(r, 'saldó');
});

await test('Préstamos', 'consultar_prestamo existente', async () => {
  const r = await processAction({ type:'consultar_prestamo', name:'Claudio' }, mockData(), UID, NAME);
  assertIncludes(r, 'Claudio');
});

await test('Préstamos', 'consultar_prestamo inexistente', async () => {
  const r = await processAction({ type:'consultar_prestamo', name:'Nadie' }, mockData(), UID, NAME);
  assertIncludes(r, 'encontr');
});

await test('Préstamos', 'consultar_todos_prestamos con datos', async () => {
  const r = await processAction({ type:'consultar_todos_prestamos' }, mockData(), UID, NAME);
  assertIncludes(r, 'Claudio');
});

await test('Préstamos', 'consultar_todos_prestamos vacío', async () => {
  const r = await processAction({ type:'consultar_todos_prestamos' }, mockData({ loans:[] }), UID, NAME);
  assertIncludes(r, 'No tenés');
});

// ── Deudas ────────────────────────────────────────────────
await test('Deudas', 'agregar_deuda', async () => {
  const r = await processAction({ type:'agregar_deuda', name:'Tarjeta Naranja', remaining:40000, installment:8000 }, mockData(), UID, NAME);
  assertIncludes(r, 'Naranja'); assertIncludes(r, '40.000');
});

await test('Deudas', 'pagar_deuda parcial', async () => {
  const r = await processAction({ type:'pagar_deuda', keyword:'Visa', amount:16000 }, mockData(), UID, NAME);
  assertIncludes(r, '16.000');
});

await test('Deudas', 'pagar_deuda que salda', async () => {
  const r = await processAction({ type:'pagar_deuda', keyword:'Visa', amount:80000 }, mockData(), UID, NAME);
  assertIncludes(r, 'saldada');
});

await test('Deudas', 'pagar_deuda inexistente', async () => {
  const r = await processAction({ type:'pagar_deuda', keyword:'XYZ', amount:1000 }, mockData(), UID, NAME);
  assertIncludes(r, 'encontr');
});

await test('Deudas', 'consultar_deudas con datos', async () => {
  const r = await processAction({ type:'consultar_deudas' }, mockData(), UID, NAME);
  assertIncludes(r, 'Visa');
});

await test('Deudas', 'consultar_deudas vacío', async () => {
  const r = await processAction({ type:'consultar_deudas' }, mockData({ debts:[] }), UID, NAME);
  assertIncludes(r, 'No tenés');
});

// ── Ahorros ───────────────────────────────────────────────
await test('Ahorros', 'agregar_ahorro', async () => {
  const r = await processAction({ type:'agregar_ahorro', name:'Moto', target:500000, current:0 }, mockData(), UID, NAME);
  assertIncludes(r, 'Moto'); assertIncludes(r, '500.000');
});

await test('Ahorros', 'depositar_ahorro parcial', async () => {
  const r = await processAction({ type:'depositar_ahorro', keyword:'vacaciones', amount:20000 }, mockData(), UID, NAME);
  assertIncludes(r, '20.000'); assertIncludes(r, 'Vacaciones');
});

await test('Ahorros', 'depositar_ahorro completa meta', async () => {
  const r = await processAction({ type:'depositar_ahorro', keyword:'vacaciones', amount:200000 }, mockData({ savings:[{ id:'s1', name:'Vacaciones', target:200000, current:10000, history:[] }] }), UID, NAME);
  assertIncludes(r, 'cumplida');
});

await test('Ahorros', 'depositar_ahorro inexistente', async () => {
  const r = await processAction({ type:'depositar_ahorro', keyword:'XYZ', amount:1000 }, mockData(), UID, NAME);
  assertIncludes(r, 'encontr');
});

await test('Ahorros', 'consultar_ahorros con datos', async () => {
  const r = await processAction({ type:'consultar_ahorros' }, mockData(), UID, NAME);
  assertIncludes(r, 'Vacaciones');
});

await test('Ahorros', 'consultar_ahorros vacío', async () => {
  const r = await processAction({ type:'consultar_ahorros' }, mockData({ savings:[] }), UID, NAME);
  assertIncludes(r, 'No tenés');
});

// ── Gastos Fijos ──────────────────────────────────────────
await test('Gastos fijos', 'agregar_gasto_fijo', async () => {
  const r = await processAction({ type:'agregar_gasto_fijo', description:'Gym', amount:15000, category:'Salud', day:5 }, mockData(), UID, NAME);
  assertIncludes(r, 'Gym'); assertIncludes(r, '15.000');
});

await test('Gastos fijos', 'actualizar por nombre', async () => {
  const r = await processAction({ type:'actualizar_gasto_fijo', keyword:'internet', day:10 }, mockData(), UID, NAME);
  assertIncludes(r, 'día → 10');
});

await test('Gastos fijos', 'actualizar todos', async () => {
  const r = await processAction({ type:'actualizar_gasto_fijo', keyword:'todos', day:5 }, mockData(), UID, NAME);
  assertIncludes(r, 'todos los gastos fijos');
});

await test('Gastos fijos', 'eliminar_gasto_fijo', async () => {
  const r = await processAction({ type:'eliminar_gasto_fijo', keyword:'nafta' }, mockData(), UID, NAME);
  assertIncludes(r, 'desactivé');
});

await test('Gastos fijos', 'registrar pide confirmación', async () => {
  const r = await processAction({ type:'registrar_gastos_fijos', date:today() }, mockData(), UID, NAME, [], PHONE);
...
await test('Gastos fijos', 'registrar sin gastos configurados', async () => {
  const r = await processAction({ type:'registrar_gastos_fijos', date:today() }, mockData({ recurringExpenses:[] }), UID, NAME, [], PHONE);
  assertIncludes(r, 'No tenés');
});

// ── Ingresos recurrentes ───────────────────────────────────
await test('Ingresos recurrentes', 'agregar_ingreso_recurrente', async () => {
  const r = await processAction({ type:'agregar_ingreso_recurrente', name:'Juan', amount:50000, reason:'alquiler', day:5 }, mockData(), UID, NAME);
...
await test('Vocabulario', 'guardar_vocabulario nuevo', async () => {
  const r = await processAction({ type:'guardar_vocabulario', expresion:'chino', descripcion:'Chino del barrio', categoria:'Comida' }, mockData(), UID, NAME);
...
await test('Vocabulario', 'guardar_vocabulario sobreescribe existente', async () => {
  const r = await processAction({ type:'guardar_vocabulario', expresion:'super', descripcion:'Super chino', categoria:'Comida' }, mockData(), UID, NAME);
  assertIncludes(r, 'super');
});

await test('Vocabulario', 'confirmar_vocabulario guarda pending', async () => {
  const r = await processAction({ type:'confirmar_vocabulario', expresion:'gym', interpretacion:'Gimnasio', categoria:'Salud', tx:{ txType:'gasto', description:'Gimnasio', amount:5000, category:'Salud', date:today() } }, mockData(), UID, NAME, [], PHONE);
...
await test('Edge cases', 'conversacion devuelve respuesta directa', async () => {
  const r = await processAction({ type:'conversacion', respuesta:'Hola!' }, mockData(), UID, NAME);
  assert(r === 'Hola!', `Esperaba "Hola!", recibí "${r}"`);
});

await test('Edge cases', 'conversacion sin respuesta da fallback', async () => {
  const r = await processAction({ type:'conversacion', respuesta:'' }, mockData(), UID, NAME);
  assertStr(r);
});

await test('Edge cases', 'type unknown no crashea', async () => {
  const r = await processAction({ type:'unknown' }, mockData(), UID, NAME);
  assertStr(r);
});

await test('Edge cases', 'action desconocida no crashea', async () => {
  const r = await processAction({ type:'accion_inexistente' }, mockData(), UID, NAME);
  assertStr(r);
});
